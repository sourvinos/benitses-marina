...
namespace BoatTypes {
    ...
    [Collection("Sequence")]
    public class BoatTypes02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        private readonly string _url = "/boatTypes/getForBrowser";

        #endregion

        public BoatTypes02GetForBrowser(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<BoatTypeBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
