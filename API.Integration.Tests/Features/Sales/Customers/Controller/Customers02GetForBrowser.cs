...
namespace Customers {
    ...
    public class Customers02GetForBrowser : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Active_Users_Can_Get_Active(Login login) {
            ...
            var records = JsonSerializer.Deserialize<List<CustomerBrowserVM>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
